...
function ProfileSkeleton() {
  const s = { backgroundImage: 'linear-gradient(90deg,var(--bg-3) 25%,var(--bg-4) 50%,var(--bg-3) 75%)', backgroundSize: '200% 100%', animation: 'shimmer 1.4s infinite', borderRadius: 10 };
  return (
...
function MiniCard({ project, onClick, index }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{
        background: 'var(--bg-2)', border: `1px solid ${hov ? 'rgba(138,111,71,0.4)' : 'var(--border)'}`,
        borderRadius: 16, overflow: 'hidden', cursor: 'pointer',
        transition: 'all 0.3s cubic-bezier(0.16,1,0.3,1)',
        transform: hov ? 'translateY(-5px) scale(1.01)' : 'translateY(0)',
        boxShadow: hov ? '0 16px 48px rgba(58,49,38,0.10), 0 0 0 1px rgba(138,111,71,0.15)' : '0 2px 8px rgba(58,49,38,0.06)',
      ...
      <div style={{ height: 140, background: 'var(--bg-3)', overflow: 'hidden', position: 'relative' }}>
        {img
          ...
          : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30, fontFamily: 'var(--mono)', color: 'rgba(138,111,71,0.2)', fontWeight: 300, letterSpacing: '-0.05em', transition: 'transform 0.5s', transform: hov ? 'scale(1.06)' : 'scale(1)' }}>&lt;/&gt;</div>
        }
        {img && <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to bottom, transparent 55%, rgba(28,24,19,0.34) 100%)', pointerEvents: 'none' }} />}
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 1, background: 'rgba(31,28,24,0.08)', pointerEvents: 'none' }} />
      </div>
      <div style={{ padding: '12px 14px 14px' }}>
        <div style={{ fontSize: 16, fontWeight: 600, fontFamily: 'var(--display)', letterSpacing: '-0.01em', color: 'var(--text-primary)', marginBottom: 9, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{project.title}</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {project.status && (
            <span style={{ marginLeft: 'auto', fontSize: 9, fontWeight: 700, fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '2px 8px', borderRadius: 99, background: project.status === 'published' ? 'rgba(63,125,94,0.12)' : 'rgba(176,138,62,0.12)', color: project.status === 'published' ? '#3f7d5e' : '#b08a3e' }}>
...
function UserListDrawer({ title, list, loading, onClose, onNavigate }) {
  ...
  return (
    <>
      <div onClick={close} style={{ position: 'fixed', inset: 0, background: 'rgba(46,40,32,0.32)', backdropFilter: 'blur(6px)', zIndex: 200, opacity: vis ? 1 : 0, transition: 'opacity 0.3s' }} />
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '100%', maxWidth: 380, background: 'var(--bg-1)', borderLeft: '1px solid var(--border)', zIndex: 201, display: 'flex', flexDirection: 'column', transform: vis ? 'translateX(0)' : 'translateX(100%)', transition: 'transform 0.35s cubic-bezier(0.16,1,0.3,1)', boxShadow: '-16px 0 60px rgba(58,49,38,0.12)' }}>
        {/* Header */}
          ...
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {Array.from({ length: 4 }, (_, i) => (
                <div key={i} style={{ height: 60, borderRadius: 12, backgroundImage: 'linear-gradient(90deg,var(--bg-3) 25%,var(--bg-4) 50%,var(--bg-3) 75%)', backgroundSize: '200% 100%', animation: `shimmer 1.4s infinite ${i * 0.1}s` }} />
              ))}
            </div>
          ) : list.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '48px 20px', color: 'var(--text-muted)' }}>
              <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 14, color: 'var(--text-muted)' }}><Icon name="users" size={24} /></div>
...
function EditDrawer({ profileData, onClose, onSaved }) {
  ...
  const inputCss = { width: '100%', height: 48, padding: '0 14px', background: 'rgba(255,255,255,0.86)', border: '1px solid rgba(31,28,24,0.11)', borderRadius: 12, color: 'var(--text-primary)', fontSize: 14, fontFamily: 'var(--sans)', outline: 'none', transition: 'all 0.2s' };
  const onFocus = e => { e.target.style.borderColor = 'var(--accent)'; e.target.style.background = '#ffffff'; e.target.style.boxShadow = '0 0 0 3px rgba(138,111,71,0.12)'; };
  const onBlur  = e => { e.target.style.borderColor = 'rgba(31,28,24,0.09)'; e.target.style.background = 'rgba(255,255,255,0.86)'; e.target.style.boxShadow = 'none'; };
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                ...
                <div style={{ position: 'relative', flexShrink: 0 }}>
                  <div style={{ width: 80, height: 80, borderRadius: '50%', overflow: 'hidden', border: '3px solid transparent', backgroundImage: currentSrc ? 'none' : 'linear-gradient(var(--bg-2),var(--bg-2)), linear-gradient(135deg,var(--accent),var(--accent-2))', backgroundOrigin: 'border-box', backgroundClip: currentSrc ? 'border-box' : 'padding-box, border-box', display: 'flex', alignItems: 'center', justifyContent: 'center', background: currentSrc ? 'transparent' : undefined, boxShadow: '0 4px 20px rgba(138,111,71,0.3)' }}>
                    {currentSrc
                ...
                <div
                  ...
                  style={{ flex: 1, height: 80, borderRadius: 14, border: `2px dashed ${dragOver ? 'var(--accent)' : 'rgba(31,28,24,0.11)'}`, background: dragOver ? 'rgba(138,111,71,0.08)' : 'rgba(31,28,24,0.02)', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, transition: 'all 0.2s', color: 'var(--text-muted)' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(138,111,71,0.4)'; e.currentTarget.style.background = 'rgba(138,111,71,0.05)'; }}
                  onMouseLeave={e => { if (!dragOver) { e.currentTarget.style.borderColor = 'rgba(31,28,24,0.11)'; e.currentTarget.style.background = 'rgba(31,28,24,0.02)'; } }}>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input value={form.location} onChange={e => setForm(f => ({ ...f, location: e.target.value }))} placeholder="Bogotá, Colombia" style={{ ...inputCss, padding: '0 14px 0 38px' }} onFocus={onFocus} onBlur={onBlur} />
              </div>
            </div>
          </div>
        </div>
...
export function ProfilePage({ userId: propUserId, onNavigate }) {
  ...
  if (!profileData) return (
    <div style={{ textAlign: 'center', padding: '80px 20px' }}>
      <div style={{ width: 62, height: 62, borderRadius: '50%', margin: '0 auto 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.72)', border: '1px solid rgba(31,28,24,0.08)', boxShadow: '0 2px 12px rgba(58,49,38,0.06)', color: 'var(--accent)' }}><Icon name={'user'} size={24} /></div>
      <h3 style={{ fontSize: 22, fontWeight: 500, fontFamily: 'var(--display)', letterSpacing: '-0.015em', color: 'var(--text-primary)' }}>Usuario no encontrado</h3>
    </div>
  );
  ...
  function StatNum({ value, label, onClick }) {
    ...
    return (
      <div onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
        style={{ textAlign: 'center', cursor: onClick ? 'pointer' : 'default', transition: 'transform 0.2s', transform: hov && onClick ? 'translateY(-2px)' : 'translateY(0)', padding: '8px 16px', borderRadius: 12, background: hov && onClick ? 'rgba(31,28,24,0.035)' : 'transparent' }}>
        <div style={{ fontSize: 26, fontWeight: 600, fontFamily: 'var(--display)', color: hov && onClick ? 'var(--accent)' : 'var(--text-primary)', lineHeight: 1, marginBottom: 5, letterSpacing: '-0.02em', transition: 'color 0.2s' }}>{value ?? 0}</div>
        <div className="eyebrow" style={{ fontSize: 9.5, color: hov && onClick ? 'var(--text-secondary)' : 'var(--text-muted)', textDecoration: onClick ? 'underline dotted' : 'none', textUnderlineOffset: 4 }}>{label}</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', animation: 'fadeIn 0.4s ease' }}>
      {/* Banner decorativo: losa de mármol veteada */}
      <div style={{ height: 200, position: 'relative', overflow: 'hidden', flexShrink: 0,
        background: 'linear-gradient(140deg, #f2ece2 0%, #e9e1d4 38%, #f7f3ec 68%, #e4dbcb 100%)',
        borderBottom: '1px solid rgba(31,28,24,0.08)' }}>
        {/* Vetas */}
        <div style={{ position: 'absolute', inset: 0, opacity: 0.7, backgroundImage:
          'repeating-linear-gradient(108deg, transparent 0 54px, rgba(120,110,96,0.08) 54px 55px, transparent 55px 130px),' +
          'repeating-linear-gradient(64deg, transparent 0 120px, rgba(150,122,78,0.10) 120px 121.5px, transparent 121.5px 260px)' }} />
        {/* Halos de luz */}
        <div style={{ position: 'absolute', top: -60, left: '18%', width: 260, height: 260, borderRadius: '50%', background: 'radial-gradient(circle, rgba(255,255,255,0.85) 0%, transparent 70%)', filter: 'blur(24px)' }} />
        <div style={{ position: 'absolute', bottom: -80, right: '12%', width: 220, height: 220, borderRadius: '50%', background: 'radial-gradient(circle, rgba(194,168,124,0.24) 0%, transparent 70%)', filter: 'blur(26px)' }} />
        {/* Filete inferior en latón */}
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 1, background: 'linear-gradient(90deg, transparent, rgba(138,111,71,0.45), rgba(194,168,124,0.30), transparent)' }} />
        {/* Botón editar sobre el banner */}
        {isOwn && (
          <button onClick={() => setShowEdit(true)} style={{ position: 'absolute', top: 18, right: 18, display: 'flex', alignItems: 'center', gap: 7, padding: '9px 17px', borderRadius: 11, background: 'rgba(255,253,250,0.78)', backdropFilter: 'blur(14px)', border: '1px solid rgba(31,28,24,0.10)', color: 'var(--text-primary)', fontSize: 12.5, fontWeight: 500, cursor: 'pointer', fontFamily: 'var(--sans)', boxShadow: '0 2px 10px rgba(58,49,38,0.08)', transition: 'all 0.24s cubic-bezier(0.16,1,0.3,1)' }}
            onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,253,250,0.98)'; e.currentTarget.style.borderColor = 'rgba(138,111,71,0.35)'; e.currentTarget.style.transform = 'translateY(-1px)'; }}
            onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,253,250,0.78)'; e.currentTarget.style.borderColor = 'rgba(31,28,24,0.10)'; e.currentTarget.style.transform = 'translateY(0)'; }}>
      ...
      <div className="page-body" style={{ maxWidth: 940, margin: '0 auto', paddingBottom: 0 }}>
        {/* Avatar + acciones */}
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginTop: -52, marginBottom: 20, gap: 16 }}>
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            <div style={{
              ...
              background: picSrc ? 'transparent' : 'linear-gradient(140deg, rgba(138,111,71,0.16), rgba(194,168,124,0.12))',
              overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center',
              boxShadow: '0 10px 30px rgba(58,49,38,0.14), 0 0 0 1px rgba(31,28,24,0.06)',
            ...
            {/* Indicador online */}
            {isOwn && <div style={{ position: 'absolute', bottom: 6, right: 4, width: 16, height: 16, borderRadius: '50%', background: 'var(--green)', border: '3px solid #fffdfa', boxShadow: '0 2px 6px rgba(63,125,94,0.35)' }} />}
          </div>

          {/* Botón seguir (no propio) */}
          {!isOwn && user && (
            <button onClick={toggleFollow} disabled={followLoad}
              style={{ padding: '10px 24px', borderRadius: 12, border: following ? '1.5px solid rgba(31,28,24,0.16)' : 'none', background: following ? 'rgba(31,28,24,0.045)' : 'linear-gradient(135deg,var(--accent),#6d5735)', color: following ? 'var(--text-secondary)' : '#fff', fontSize: 14, fontWeight: 700, cursor: followLoad ? 'not-allowed' : 'pointer', fontFamily: 'var(--sans)', display: 'flex', alignItems: 'center', gap: 8, transition: 'all 0.25s', boxShadow: following ? 'none' : '0 6px 20px rgba(138,111,71,0.4)', alignSelf: 'center', flexShrink: 0 }}
              onMouseEnter={e => { if (!followLoad) { e.currentTarget.style.transform = 'translateY(-2px)'; } }}
              onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; }}>
              {followLoad
                ? <div style={{ width: 16, height: 16, border: '2px solid rgba(255,255,255,0.55)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.65s linear infinite' }} />
                : following
        ...
        <div style={{ marginBottom: 24 }}>
          <h1 className="page-title" style={{ fontSize: 36, margin: '0 0 8px' }}>{profileData.fullName}</h1>
        ...
        {tab === 'projects' && (
          projects.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px', animation: 'fadeIn 0.4s ease' }}>
              <div style={{ width: 62, height: 62, borderRadius: '50%', margin: '0 auto 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.72)', border: '1px solid rgba(31,28,24,0.08)', boxShadow: '0 2px 12px rgba(58,49,38,0.06)', color: 'var(--accent)' }}><Icon name={'terminal'} size={24} /></div>
              <h3 style={{ fontSize: 20, fontWeight: 500, fontFamily: 'var(--display)', letterSpacing: '-0.015em', color: 'var(--text-primary)', marginBottom: 8 }}>Sin proyectos publicados</h3>
              {isOwn && (
                <button onClick={() => onNavigate('my-projects')} style={{ marginTop: 12, padding: '10px 24px', borderRadius: 12, background: 'linear-gradient(135deg,var(--accent),#6d5735)', border: 'none', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--sans)' }}>
          ...
          <div style={{ maxWidth: 520, paddingBottom: 80, animation: 'fadeIn 0.35s ease' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: 16, padding: '16px 0', borderBottom: '1px solid var(--border)', animation: `slideUp 0.4s cubic-bezier(0.16,1,0.3,1) ${i * 0.08}s both` }}>
                <div style={{ width: 38, height: 38, borderRadius: 10, background: 'rgba(138,111,71,0.1)', border: '1px solid rgba(138,111,71,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--accent)', flexShrink: 0 }}>
      ...
      {showEdit        && <EditDrawer profileData={profileData} onClose={() => setShowEdit(false)} onSaved={() => { load(); refreshUser(); }} />}
    </div>
  );
}
