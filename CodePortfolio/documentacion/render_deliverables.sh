...
SOURCE_DIR="/Users/agent/CodePortfolio/CodePortfolio/documentacion/entregables_ieee"
DESKTOP_DIR="/Users/agent/Desktop/Entregables_IEEE_CodePortfolio_2026"
...
for html_file in "$SOURCE_DIR"/*.html; do
  ...
  textutil -convert docx \
    -title "CodePortfolio — Entregable de calidad" \
    -author "Pedro Bonilla" \
    -company "CodePortfolio" \
    -output "$docx_temp" "$html_file"
  mv -f "$docx_temp" "$docx_file"

  chrome_profile="$(mktemp -d /private/tmp/codeportfolio-render.XXXXXX)"
  "$CHROME_BIN" \
    --headless=new \
    --disable-gpu \
